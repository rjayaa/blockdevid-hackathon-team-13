...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function verifyRole() {
      ...
      try {
        const hasRole = await checkVerifierRole(account.address);
        setIsVerifier(hasRole);

        if (!hasRole) {
          alert("Access Denied: You don't have VERIFIER_ROLE");
          router.push("/(main)/dashboard");
        }
      } catch (error) {
        console.error("Error verifying role:", error);
        alert("Error verifying your role. Redirecting...");
        router.push("/(main)/dashboard");
      } finally {
        setIsChecking(false);
      }
  ...
  const handleMint = (e: React.FormEvent) => {
    e.preventDefault();
    alert(
      "Dummy Action: Project Registered, Metadata Uploaded to Pinata (Simulation)",
    );
  };
          ...
          <CardContent>
            <form onSubmit={handleMint} className="space-y-6">
              ...
              <div className="space-y-4 p-4 border border-foreground/50 bg-carbon-light/50">
                ...
                <Input
                  id="name"
                  placeholder="Mangrove Restoration Delta"
                ...
                <Input
                  id="location"
                  placeholder="Kalimantan Barat, Indonesia"
                  className="bg-foreground border-foreground"
                />
              ...
              <div className="space-y-4 p-4 border border-foreground/50 bg-carbon-light/50">
                ...
                <Input
                  id="file"
                  type="file"
                  className="bg-foreground border-foreground file:text-carbon-primary"
                />
              </div>
